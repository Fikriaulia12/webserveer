...
// Validate the data 
if (!isset($data->name) || !isset($data->email) || !isset($data->prodi) || 
!isset($data->fakultas)) {     die(json_encode(["error" => "Invalid input"])); 
} 
...
$sql = "INSERT INTO users (name, email, prodi, fakultas) VALUES ('$name', 
'$email', '$prodi', '$fakultas')"; 
 if ($koneksi->query($sql) === TRUE) {     echo json_encode(["success" => true]); 
} else {     echo json_encode(["error" => $koneksi->error]); 
} 
